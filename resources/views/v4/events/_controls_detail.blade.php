...
@php
  if (Route::has('r4.events.edit')) {
    $edit_href = route('r4.events.edit', ['event' => $event]);
  }

  if (Route::has('r4.events.edit_raw')) {
    $edit_raw_href = route('r4.events.edit_raw', ['event' => $event]);
  }

  if (Route::has('r4.events.destroy')) {
    $destroy_href = route('r4.events.destroy', ['event' => $event]);
  }

  // 左ナビ表示の場合はリンク先を変える
  if (isset($is_lnav)) {
    $edit_href = route('r4.events.lnav', ['event' => $event, 'edit_mode' => 'edit'], $conds);
  }
@endphp
...
@if (($show_delete_button ?? false) && ($destroy_href ?? false))
  <div class="row mt-5">
    ...
    <div class="col-auto">
      <x-button.destroy :url="$destroy_href" class="" label="このイベントを削除" />
    </div>
  </div>
@endif
