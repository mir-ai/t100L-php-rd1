...
class F503SqlRawEventTest extends TestCase
{
    ...
    public function test_503_030_event_count_by_date()
    {
        ...
        // QUIZ
        $rows = DB::select('
           select date_format(open_date, \'%Y年%m月%d日\') as ymd,
                  count(*) as cnt
             from events
            group by ymd
            order by cnt desc
    ...
    public function test_503_050_filter_hamamatsu_event_names()
    {
        ...
        $rows = DB::select('
           select event_name
             from events
            where (
                event_name like \'%浜松%\' or
                event_name like \'%ハママツ%\' or
                event_name like \'%はままつ%\' or
                event_name like \'%HAMAMATSU%\' or
                event_name like \'%Hamamatsu%\' or
                event_name like \'%hamamatu%\' 
            )
            order by event_name
        ');
        // /QUIZ
