 ...
 * 
 */
class F514SqlEloquentPopulationTest extends TestCase
{
    // 0歳児の多い順に町を３件出力してみよう
    ...
    public function test_504_020_oldest_city_ranking(): void
    {
        ...
        $rows = Population::query()
        ->select('oaza_name')
        ->selectRaw('(sum(total_count * age) / sum(total_count)) as avg_age')
        ->groupBy('oaza_name')
    ...
    public function test_504_030_average_age_by_gender(): void
    {
        ...
        // QUIZ
        $rows = Population::query()
        ->selectRaw('(sum(male_count * age) / sum(male_count)) as avg_male_age')
        ->selectRaw('(sum(female_count * age) / sum(female_count)) as avg_female_age')
        ->get();
        // /QUIZ
