 ...
 * 
 */
class F513SqlEloquentEventTest extends TestCase
{
    // 浜松市で一番イベントが開催されている場所(location_name)を上位3件抽出する
    public function test_513_010_popular_places(): void
    {
        $rows = [];

        // QUIZ
        $rows = Event::select('location_name')
    ...
    public function test_513_020_event_count_by_wards()
    {
        $rows = [];

        // QUIZ
        $rows = Event::select('ward_name')
            ->selectRaw('count(*) as cnt')
            ->groupBy('ward_name')
            ->orderBy('cnt', 'desc')
            ->limit(3)
    ...
    public function test_513_050_filter_hamamatsu_event_names()
    {
        $rows = [];
        ...
        $rows = Event::query()
        ->select('event_name')
        ->where('event_name', 'like', '%浜松%')
        ->orWhere('event_name', 'like', '%ハママツ%')
        ->orWhere('event_name', 'like', '%はままつ%')
        ->orWhere('event_name', 'like', '%HAMAMATSU%')
        ->orWhere('event_name', 'like', '%Hamamatsu%')
        ->orWhere('event_name', 'like', '%hamamatu%')
        ->orderBy('event_name')
        ->get();
        // /QUIZ
